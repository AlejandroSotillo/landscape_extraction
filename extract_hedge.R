...
#                 e.g. 'F:/OneDrive/work_conc/data/env/bocage'
#
# path_dept       full file path, name and extension for the france_political layer
#                 e.g. "F:/OneDrive/work_conc/data/political/france_political/france_political_20220728.RData"
#                 
#
...
extract_hedge <- function(dt,
  ...
  for(i in unique(dt_sf$code_dept)){
    ...
    # add distance to closest hedge
    
    dt_sf[dt_sf$code_dept == i,]$distance_hedge <- st_distance(dt_sf[dt_sf$code_dept == i,],
                                                               hedge[st_nearest_feature(dt_sf[dt_sf$code_dept == i,], hedge),], by_element = T)
  ...
  thevars = c(id_field,'distance_hedge')
  
  dt_product <- merge(x = dt_product,
                      y = as.data.table(dt_sf)[,..thevars],
                      by = id_field,
                      all.x = T, all.y = F)
